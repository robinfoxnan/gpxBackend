...
func NewConcurrentMap[K comparable, V any]() ConcurrentMap[K, V] {
	// create slice of len shard_count
	m := make(ConcurrentMap[K, V], SHARD_COUNT)
...
// a hash function simply
func stringKey32(key string) uint32 {
...
// should here ,user interface to get data not reflect!!!
func (m ConcurrentMap[K, V]) GetShard(key K) *ConcurrentMapShared[K, V] {

	var localKey uint = 0

	var ti interface{} = key
	switch ti.(type) {
	case int:
		localKey = uint(ti.(int))
	case uint:
		localKey = ti.(uint)
	case int32:
		localKey = uint(ti.(int32))
	case uint32:
		localKey = uint(ti.(uint32))
	case uint64:
		localKey = uint(ti.(uint64))
	case int64:
		localKey = uint(ti.(int64))
	case string:
		localKey = uint(stringKey32(ti.(string)))
	default:
		localKey = 0
	}

	//fmt.Println("local key is: ", localKey)
	return m[localKey%uint(SHARD_COUNT)]
}
...
// here, suitable for increase some counter and so on
